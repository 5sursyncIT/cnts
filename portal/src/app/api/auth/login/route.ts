...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || "https://cnts.5sursync.com"; // Default to prod URL for now
...
export async function POST(request: Request) {
  ...
  if (isRateLimited) {
    const url = new URL("/espace-patient/connexion", APP_URL);
  ...
  try {
    ...
    if (res.status === 401) {
      return NextResponse.redirect(new URL(`/espace-patient/connexion?error=1&next=${encodeURIComponent(safeNext)}`, APP_URL));
    }

    if (!res.ok) {
      logger.error({ status: res.status, body: await res.text() }, "Login error");
      return NextResponse.redirect(new URL(`/espace-patient/connexion?error=system&next=${encodeURIComponent(safeNext)}`, APP_URL));
    }

    const data = await res.json();

    if (data.mfa_required) {
      return NextResponse.redirect(new URL(`/espace-patient/connexion?error=mfa_not_supported&next=${encodeURIComponent(safeNext)}`, APP_URL));
    }
